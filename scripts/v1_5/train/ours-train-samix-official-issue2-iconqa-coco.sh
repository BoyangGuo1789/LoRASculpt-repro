...
COCO_SAMPLES=${COCO_SAMPLES:-1500}
...
TRAINER_NAME=${TRAINER_NAME:-LoRASculpt}
OUTPUT_ROOT=${OUTPUT_ROOT:-$ROOT/checkpoints}
RUN_NAME=${RUN_NAME:-llava-v1.5-7b-lorasculpt-samix-coco${COCO_SAMPLES}-iconqa-r${LORA_RANK}}
OUTPUT_DIR_SUFFIX=""
extra_args=()
...
echo "[SA-MIX] data_path=$SAMIX_JSON"
echo "[SA-MIX] manifest=$SAMIX_MANIFEST"
echo "[SA-MIX] image_folder=$IMAGE_FOLDER_OVERRIDE"
echo "[SA-MIX] output_dir=$OUTPUT_DIR"
echo "[SA-MIX] trainer=$TRAINER_NAME rank=$LORA_RANK alpha=$LORA_ALPHA"
echo "[SA-MIX] coco_samples=$COCO_SAMPLES seed=$SAMIX_SEED shuffle_seed=$SAMIX_SHUFFLE_SEED"
echo "[SA-MIX] step_threshold=$STEP_THRESHOLD extra_args=${extra_args[*]:-}"
