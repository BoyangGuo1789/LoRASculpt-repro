...
source ./scripts/v1_5/train/trainconfig_ids_lora.sh
...
export TRAINER_NAME="LoRASculptDIS"
...
echo "[LoRASculpt-DIS] data_path=$DATA_PATH_OVERRIDE"
echo "[LoRASculpt-DIS] image_folder=$IMAGE_FOLDER_OVERRIDE"
echo "[LoRASculpt-DIS] output_dir=$OUTPUT_DIR"
echo "[LoRASculpt-DIS] trainer=$TRAINER_NAME rank=$LORA_RANK alpha=$LORA_ALPHA"
echo "[LoRASculpt-DIS] ids_enable=$IDS_ENABLE grad_lambda=$IDS_GRAD_LAMBDA ret_lambda=$IDS_RET_LAMBDA beta=$IDS_GRAD_EMA_BETA retention=$IDS_APPLY_RETENTION_TO"
echo "[LoRASculpt-DIS] step_threshold=$STEP_THRESHOLD extra_args=${extra_args[*]:-}"
...
"$DEEPSPEED_BIN" --include "$DEVICE" --master_port "$MASTER_PORT" llava/train/train_mem.py \
    ...
    --ids_enable "$IDS_ENABLE" \
    --ids_grad_lambda "$IDS_GRAD_LAMBDA" \
    --ids_ret_lambda "$IDS_RET_LAMBDA" \
    --ids_grad_ema_beta "$IDS_GRAD_EMA_BETA" \
    --ids_apply_retention_to "$IDS_APPLY_RETENTION_TO" \
    --ids_score_eps "$IDS_SCORE_EPS" \
    "${extra_args[@]}"
