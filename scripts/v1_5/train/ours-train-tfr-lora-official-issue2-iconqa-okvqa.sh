...
INIT_LORA=${INIT_LORA:-$ROOT/checkpoints/llava-v1.5-7b-lorasculpt-tfr-init-target-r${LORA_RANK}-a${LORA_ALPHA}}
LOAD_NON_LORA=${LOAD_NON_LORA:-True}

SOURCE_WEIGHT=${SOURCE_WEIGHT:-1.0}
LAMBDA_TARGET_KL=${LAMBDA_TARGET_KL:-1.0}
...
RUN_NAME=${RUN_NAME:-llava-v1.5-7b-lorasculpt-tfr-okvqa-s${OKVQA_SAMPLES}-sw${SOURCE_WEIGHT}-tkl${LAMBDA_TARGET_KL}-r${LORA_RANK}}
...
echo "[TFR-LoRA] data_path=$MIX_JSON"
echo "[TFR-LoRA] manifest=$MIX_MANIFEST"
echo "[TFR-LoRA] image_folder=$ROOT/data"
echo "[TFR-LoRA] output_dir=$OUTPUT_DIR"
